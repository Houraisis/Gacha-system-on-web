...
function getRandomItem() {
    ...
    // 가중치 설정
    const totalWeight = Math.round(Object.values(rarityProbabilities).reduce((acc, val) => acc + val, 0));
    ...
    for (let rarity in rarityProbabilities) {
        ...
        if (random <= 0) {
            const filtered_list = availableItems.filter(item => item.rarity === rarity);
            const result = filtered_list[Math.floor(Math.random() * filtered_list.length)];
...
function generateItemsForRound() {
    ...
    while (items.length < 3) {
        ...
        if (items.length === 2 && item.rarity !== "legendary" && leg_count < MIN_LEG && players[currentPlayerIndex].pickedItems.length === 4) { // 하드코딩됨 재량에 따라 수정
            continue;
        }

        if (item && !items.includes(item) && !players.some(player => player.pickedItems.includes(item))) {
            items.push(item);
        }
    }
...
function renderItems() {
    ...
    itemsForRound.forEach(item => {
        ...
        // 또한 뒷면 이미지가 나올 내용을 itemsDiv의 자식 노드로도 추가시킨다
        // 이를 통해 itemDiv이랑 flipCardBack는 서로 동등한 위치의 태그가 된다
        itemsDiv.appendChild(flipCardBack);
        anime({
